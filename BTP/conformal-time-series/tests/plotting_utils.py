...
def plot_time_series(fig, axs, time_series_list, window_start, window_end, window_loc, sets, y, color, inset, miscoverage_scatterplot, datetimes, hline=None):
    ...
    # Calculate the minimum and maximum values for the axes and axins
    def calculate_min_max(time_series_list, window_start, window_end):
        minval = np.inf
        maxval = -np.inf
        for time_series in time_series_list:
            series_slice = time_series[window_start:window_end]
            if len(series_slice) > 0 and not np.all(np.isnan(series_slice)):
                minval = min(minval, np.nanmin(series_slice))
                maxval = max(maxval, np.nanmax(series_slice))
        return minval, maxval

    minval_ax, maxval_ax = calculate_min_max(ts_list_finite, 0, len(datetimes))
    minval_axins, maxval_axins = calculate_min_max(ts_list_finite, window_start, window_end)
    ...
    for i, time_series in enumerate(time_series_list):
        ...
        if inset:
            # Define the inset ax based on the window location
